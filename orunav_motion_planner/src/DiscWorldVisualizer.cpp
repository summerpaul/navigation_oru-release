/**
 * @Author: Xia Yunkai
 * @Date:   2023-07-20 23:04:30
 * @Last Modified by:   Xia Yunkai
 * @Last Modified time: 2023-07-20 23:19:43
 */
/**
...
DiscWorldVisualizer::DiscWorldVisualizer(int xCels, int yCels, int scale) {
  ...
  worldDisplayImg_ =
      cvCreateImage(cvSize(imgXsize_, imgYsize_), IPL_DEPTH_8U, 3);
  // white background for clarity reasons
  cvRectangle(worldDisplayImg_, cvPoint(0, 0), cvPoint(imgXsize_, imgYsize_),
              cvScalar(255, 255, 255, 0), CV_FILLED);
  ...
  for (int x = 0; x <= xTotCels_ * scaleFactor_; x += scaleFactor_) {
    cvLine(worldDisplayImg_, cvPoint(x, 0), cvPoint(x, imgYsize_),
           cvScalar(211, 211, 211, 0), 1, 8, 0);
  }
  for (int y = 0; y <= yTotCels_ * scaleFactor_; y += scaleFactor_) {
    cvLine(worldDisplayImg_, cvPoint(0, y), cvPoint(imgXsize_, y),
           cvScalar(211, 211, 211, 0), 1, 8, 0);
  }
}
...
DiscWorldVisualizer::~DiscWorldVisualizer() {
  ...
  if (WP::SAVE_FINAL_VISUALIZATION_TO_FILE) {
    // cv::saveImage((WP::LOG_FILE.append(".PNG")).c_str(), worldDisplayImg_);
  }
    ...
    for (it_x = (*it_y).begin(); it_x != (*it_y).end(); it_x++) {
      ...
      if ((*it_x) > WP::CALCULATION_APPROXIMATION_ERROR) {
        this->drawFilledRectangle(
            x * scaleFactor_, y * scaleFactor_, x * scaleFactor_ + scaleFactor_,
            y * scaleFactor_ + scaleFactor_, 255 * (1 - (*it_x)),
            255 * (1 - (*it_x)), 255 * (1 - (*it_x)));
      }
      x++;
    }
    y++;
...
void DiscWorldVisualizer::drawGoal(Configuration* conf, int vehicleID) {
  ...
  if (WP::SAVE_FINAL_VISUALIZATION_TO_FILE && WP::LOG_LEVEL >= 10) {
    ...
    // cv::imwrite(
    //     (name.append(".").append(std::to_string(vehicleID)).append(".PNG"))
    //         .c_str(),
    //     worldDisplayImg_);
  }
}

void DiscWorldVisualizer::drawConfigurations(
    std::vector<Configuration*> confs) {
  for (std::vector<Configuration*>::iterator it = confs.begin();
       it != confs.end(); it++) {
  ...
  while (threadStop_ == false) {
    ...
    if (WP::SAVE_FINAL_VISUALIZATION_TO_FILE && WP::LOG_LEVEL >= 10) {
      if (counter % 20 == 0) {
        std::string name = std::string(WP::LOG_FILE);
        // cvSaveImage(
        //     (name.append(std::to_string(counter)).append(".PNG")).c_str(),
        //     worldDisplayImg_);
      }
      counter++;
    }
...
void DiscWorldVisualizer::stopVisualization() { threadStop_ = true; }

void DiscWorldVisualizer::drawConfiguration(Configuration* conf, int R, int G,
                                            int B) {
...
void DiscWorldVisualizer::drawBaseConfiguration(Configuration* conf, int R,
                                                int G, int B) {
  // calculate the space
  int radius = 1;
  double x_centre, y_centre, x_top, y_top, x_bottom, y_bottom, x_left, y_left,
      x_right, y_right, orient, base_orient;
  if (scaleFactor_ > 2) {
  ...
  x_centre =
      conf->getXCoordinate() * (scaleFactor_ / WP::WORLD_SPACE_GRANULARITY);
  y_centre =
      conf->getYCoordinate() * (scaleFactor_ / WP::WORLD_SPACE_GRANULARITY);
...
void DiscWorldVisualizer::drawCarConfiguration(CarConfiguration* conf, int R,
                                               int G, int B) {
  ...
  double base_orient = addAnglesRadians(conf->getOrientation(), M_PI_2,
                                        WP::DECIMAL_APPROXIMATION);
  ...
  double x_base_centre =
      (conf->getXCoordinate() * scale) - back_length * cosOrient;
  double y_base_centre =
      (conf->getYCoordinate() * scale) - back_length * sinOrient;
  // central point of the front
  double x_top_centre =
      x_base_centre + (back_length + front_length) * cosOrient;
  double y_top_centre =
      y_base_centre + (back_length + front_length) * sinOrient;
  ...
  // front axel
  double x_front_axel_centre =
      x_base_centre + (back_length + front_length * 2 / 3) * cosOrient;
  double y_front_axel_centre =
      y_base_centre + (back_length + front_length * 2 / 3) * sinOrient;
  ...
  double x_wheel_right_top =
      right_wheel_center_x + wheel_size * cos(wheel_orient);
  double y_wheel_right_top =
      right_wheel_center_y + wheel_size * sin(wheel_orient);
  double x_wheel_right_bottom =
      right_wheel_center_x - wheel_size * cos(wheel_orient);
  double y_wheel_right_bottom =
      right_wheel_center_y - wheel_size * sin(wheel_orient);
  ...
  double x_wheel_left_top =
      left_wheel_center_x + wheel_size * cos(wheel_orient);
  double y_wheel_left_top =
      left_wheel_center_y + wheel_size * sin(wheel_orient);
  double x_wheel_left_bottom =
      left_wheel_center_x - wheel_size * cos(wheel_orient);
  double y_wheel_left_bottom =
      left_wheel_center_y - wheel_size * sin(wheel_orient);
  ...
  // draw the wheel
  this->drawLine(x_wheel_left_top, y_wheel_left_top, x_wheel_left_bottom,
                 y_wheel_left_bottom, R, G, B);
  this->drawLine(x_wheel_right_top, y_wheel_right_top, x_wheel_right_bottom,
                 y_wheel_right_bottom, R, G, B);
}

void DiscWorldVisualizer::drawLHDConfiguration(LHDConfiguration* conf, int R,
                                               int G, int B) {
  LHDModel* model =
      dynamic_cast<LHDModel*>(conf->getMission()->getVehicleModel());
  double scale = scaleFactor_ / WP::WORLD_SPACE_GRANULARITY;

  double frontBodyOrientation = conf->getOrientation();
  double rearBodyOrientation = addAnglesRadians(
      frontBodyOrientation, conf->getSteering(), WP::DECIMAL_APPROXIMATION);
  ...
  double frontBaseOrient =
      addAnglesRadians(frontBodyOrientation, M_PI_2, WP::DECIMAL_APPROXIMATION);
  double cosFrontBaseOrient = cos(frontBaseOrient);
  double sinFrontBaseOrient = sin(frontBaseOrient);

  double rearBaseOrient =
      addAnglesRadians(rearBodyOrientation, M_PI_2, WP::DECIMAL_APPROXIMATION);
  ...
  double frontLength =
      (model->getLHDFrontFromAxle() + model->getLHDFrontLength()) * scale;
  // rear body of the WheelLoader -- from the back to the joint
  double rearLength =
      (model->getLHDBackFromAxle() + model->getLHDBackLength()) * scale;
  ...
  this->drawDot((conf->getXCoordinate() * scale),
                (conf->getYCoordinate() * scale), width / 8, R, G, B);

  // compute the coordinates of the joint
  double x_joint =
      (conf->getXCoordinate() * scale) - frontAxleToJoint * cosFrontOrient;
  double y_joint =
      (conf->getYCoordinate() * scale) - frontAxleToJoint * sinFrontOrient;
  ...
  double x_rear_base_right =
      x_rear_top_right - (rearLengthDraw * cosRearOrient);
  double y_rear_base_right =
      y_rear_top_right - (rearLengthDraw * sinRearOrient);
  ...
  double x_front_top_right =
      x_front_base_right + (frontLengthDraw * cosFrontOrient);
  double y_front_top_right =
      y_front_base_right + (frontLengthDraw * sinFrontOrient);
  double x_front_top_left =
      x_front_base_left + (frontLengthDraw * cosFrontOrient);
  double y_front_top_left =
      y_front_base_left + (frontLengthDraw * sinFrontOrient);

  // front axle & joint
  // double x_axle_left = conf->getXcoordinate() * scale - ((width / 3) *
  // cosBaseOrient); double y_axle_left = conf->getYcoordinate() * scale -
  // ((width / 3) * sinBaseOrient); double x_axle_right = conf->getXcoordinate()
  // * scale + ((width / 3) * cosBaseOrient); double y_axle_right =
  // conf->getYcoordinate() * scale + ((width / 3) * sinBaseOrient);

  // draw the configuration
  this->drawLine(x_rear_top_right, y_rear_top_right, x_rear_top_left,
                 y_rear_top_left, R, G, B);
  this->drawLine(x_rear_top_right, y_rear_top_right, x_rear_base_right,
                 y_rear_base_right, R, G, B);
  this->drawLine(x_rear_base_left, y_rear_base_left, x_rear_base_right,
                 y_rear_base_right, R, G, B);
  this->drawLine(x_rear_base_left, y_rear_base_left, x_rear_top_left,
                 y_rear_top_left, R, G, B);

  this->drawLine(x_front_top_right, y_front_top_right, x_front_top_left,
                 y_front_top_left, R, G, B);
  this->drawLine(x_front_top_right, y_front_top_right, x_front_base_right,
                 y_front_base_right, R, G, B);
  this->drawLine(x_front_base_left, y_front_base_left, x_front_base_right,
                 y_front_base_right, R, G, B);
  this->drawLine(x_front_base_left, y_front_base_left, x_front_top_left,
                 y_front_top_left, R, G, B);

  this->drawLine(bottomFrontX, bottomFrontY, x_joint, y_joint, R, G, B);
  this->drawLine(topRearX, topRearY, x_joint, y_joint, R, G, B);
  // this->drawLine(x_axle_right, y_axle_right, x_axle_left, y_axle_left, R, G,
  // B);
}

void DiscWorldVisualizer::drawPath(std::vector<vehicleSimplePoint> path, int R,
                                   int G, int B) {
...
void DiscWorldVisualizer::drawLine(double xfrom, double yfrom, double xto,
                                   double yto, int R, int G, int B) {
  // reverse y axis!
  yfrom = imgYsize_ - yfrom;
  yto = imgYsize_ - yto;
  // lock the mutex
  boost::mutex::scoped_lock lock(update_mutex_);
  cvLine(worldDisplayImg_, cvPoint((int)xfrom, (int)yfrom),
         cvPoint((int)xto, (int)yto), cvScalar(R, G, B), 1, 8, 0);
}

void DiscWorldVisualizer::drawDot(double x, double y, double radius, int R,
                                  int G, int B) {
  // reverse the y axis for correct visualization
  y = imgYsize_ - y;
  // lock the mutex before writing
  boost::mutex::scoped_lock lock(update_mutex_);
  cvCircle(worldDisplayImg_, cvPoint((int)x, (int)y), radius, cvScalar(R, G, B),
           CV_FILLED);
}

void DiscWorldVisualizer::drawFilledRectangle(double xfrom, double yfrom,
                                              double xto, double yto, int R,
                                              int G, int B) {
  // reverse the y axis for correct visualization
  yfrom = imgYsize_ - yfrom;
  yto = imgYsize_ - yto;
  // lock the mutex before writing
  boost::mutex::scoped_lock lock(update_mutex_);
  cvRectangle(worldDisplayImg_, cvPoint((int)xfrom, (int)yfrom),
              cvPoint((int)xto, (int)yto), cvScalar(R, G, B), CV_FILLED);
}

void DiscWorldVisualizer::drawRectangle(double xfrom, double yfrom, double xto,
                                        double yto, int R, int G, int B) {
  // reverse the y axis for correct visualization
  yfrom = imgYsize_ - yfrom;
  yto = imgYsize_ - yto;
